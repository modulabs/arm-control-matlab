...
function tau = rne_modified_mdh(robot, a1, a2, a3, a4, a5, a6)
		...
		for j=1:n
            ...
            switch link.type
                case 'R'
                    ...
                    w_ = R*w + z0*qd(j);
		    wa_ = R*wa + z0*qda(j);
                    wd_ = R*wd + cross(R*wa,z0*qd(j)) + z0*qdd(j);
                    %v = cross(w,P) + R*v;
                    vd_ = R * (cross(wd,P) + ...
                        cross(w, cross(wa,P)) + vd);
			...
			vdC = cross(wd,Pc).' + ...
				cross(w,cross(wa,Pc)).' + vd;
			F = link.m*vdC;
			N = link.I*wd + cross(wa,link.I*w);
			Fm = [Fm F];
			Nm = [Nm N];
			if debug
